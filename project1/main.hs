...
solve m g l | m == "bfs"   = reverse $ solve' BFS.listM g i
            | m == "dfs"   = reverse $ solve' DFS.listM g i
            | m == "iddfs" = reverse $ solve' IFS.listM g i
...
solve' f g i = case find (MC.isGoal g) $ zip [1..] $ evalState (f MC.expand i) empty of
              Nothing         -> ["No solution."]
